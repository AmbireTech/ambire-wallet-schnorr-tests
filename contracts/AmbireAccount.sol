...
contract AmbireAccount {
	...
	function execute(Transaction[] calldata txns, bytes calldata signature)
		external
	{
        // DISCUSS: remove currentNonce and just use nonce-1 where we want to use the currentOne
        // why is this needed?
        // because a "Stack too deep" error is thrown at compile time
        nonce = nonce + 1;

		// NOTE: abi.encode is safer than abi.encodePacked in terms of collision safety
		bytes32 hash = keccak256(abi.encode(address(this), block.chainid, nonce - 1, txns));
		// We have to increment before execution cause it protects from reentrancies

		// Recovery signature: allows to perform timelocked txns
		// @TODO: cleaner mode constants?
		address signerKey = _handleScheduledAndGetSignerKeyIfAny(
            uint8(signature[signature.length - 1]),
            signature,
            hash,
            txns
        );

        if (signerKey == address(0)) return;
    ...
    function _handleScheduledAndGetSignerKeyIfAny(uint8 sigMode, bytes memory signature, bytes32 hash, Transaction[] memory txns) internal returns (address) {
        address signerKey;
        ...
        if (sigMode >= 254) {
			...
			if (scheduled != 0 && !isCancellation) {
                ...
                _execScheduled(hash, scheduled, recoveryInfoHash);
			} else {
				_deleteOrSchedule(hash, recoverySignature, recoveryInfo, isCancellation, recoveryInfoHash, txns);
			}
		} else {
    ...
    // exec the scheduled transaction
    function _execScheduled(bytes32 hash, uint scheduled, bytes32 recoveryInfoHash) internal {
    ...
    function _deleteScheduled(bytes32 hash, bytes32 recoveryInfoHash, address recoveryKey) internal {
        delete scheduledRecoveries[hash];
        emit LogCancelled(hash, recoveryInfoHash, recoveryKey, block.timestamp);
    }

    // schedule a transaction
    function _schedule(bytes32 hash, uint timelock, bytes32 recoveryInfoHash, address recoveryKey, uint currentNonce, Transaction[] memory txns) internal {
        scheduledRecoveries[hash] = block.timestamp + timelock;
        emit LogScheduled(hash, recoveryInfoHash, recoveryKey, currentNonce, block.timestamp, txns);
    }

    function _deleteOrSchedule(bytes32 hash, bytes memory recoverySignature, RecoveryInfo memory recoveryInfo, bool isCancellation, bytes32 recoveryInfoHash, Transaction[] memory txns) internal {
        ...
        if (isCancellation) {
            _deleteScheduled(hash, recoveryInfoHash, recoveryKey);
        } else {
            _schedule(hash, recoveryInfo.timelock, recoveryInfoHash, recoveryKey, nonce - 1, txns);
        }
    }
}
